...
<BnfkInterpreter
    cells="8"
    names="from Technomunk;blog;world;dear reader"
    code={`+++++ +++             Set Cell #0 to 8 to setup multiplying the following cells by 8
[
      >+++++ ++++       Add 9 to Cell #1
      >+++++ +++++ +++  Add 13 to Cell #2
      >++++             Add 4 to Cell $3
...
We could add extra instructions, but remember that we want to see how few instructions we can get away with. So instead - let's mark another cell as "command", so if it contains **1** - we tell the system to print the character from the output cell. If it contains **2** - we tell the system that we want it to update the input cell with a new character and so on. But we are still stuck with the same problem - how do we tell the system that our command is done? I would suggest adding another one (final) special cell - signal. Whenever the signal cell is updated - the system is _signalled_ that we want it to perform the command according to the command cell.
