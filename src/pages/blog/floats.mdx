...
This hardware representation leads to several interesting side-effects. For one - there are 2 zero values: a positive and a negative one. Additionally there are _subnormal_ numbers - numbers near zero that have several leading zeroes. These are equivalent to "inefficient" scientific notation: `10^10 * 0.03`. Older hardware did not handle such values well and would provide erroneous results, however nowadays they can be used without much worry.
