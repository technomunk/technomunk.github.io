...
<Code lang="lisp" theme="github-light" code={`(defun nth_fibonacci (n)
...
<Code lang="py" theme="github-light" code={`def nth_fibonacci(n):
...
Let's start with something simpler: a single *expression* <Code lang="lisp" theme="github-light" code="(+ 1 2)" inline />. As you might suspect it evaluates to 3. And that's pretty much all an expression is - a sequence of instructions that evaluates to some value. Most languages allow nesting expressions together <Code lang="lisp" theme="github-light" code="(+ (read) (read))" />. The summation is now invoking another expression (reading from input) for each of its arguments. And this nesting is why source code is usually represented as a *syntax tree* when working with it - sometimes it's sufficient to identify that "oh, this is an expression, but our syntax doesn't allow an expression here". When in doubt - expressions are whatever the language allows you to put on the right hand side of assignment.

But speaking of assignment - is it an expression? Or a function definition? Is our first set of brackets an expression? Well they don't *evaluate* to anything, so maybe not. They are just a statement - a block of code that tells what the program should do. So our first statement syntax tree looks something like:
<Code theme="github-light" code={`statement (function definition) { name: "nth_fibonacci", parameters: ["n"], doc: "Calculate the Nth Fibonacci number." }
    expression (if)
        expression (<)
            variable "n"
            value 2
...
Arguably the statement does evaluate to something - modifying the global state to define a new function that can be called. And in fact that exactly how Lisp works! So every set of brackets is an expression!

Alright, we have a weird looking data structure we have no idea how to create (and I will delve into parsing some other time), what next? How does having this tree help us? Imagine we're writing an interpreter and we had the parser that would provide us the syntax tree. How would we go about evaluating an expression?
...
<Code theme="github-light" code={`expression (+)
    expression (*)
        variable "n"
        value 2
    value -1
