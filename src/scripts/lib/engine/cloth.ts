import type { World, Entity } from './entity';
...
export class Cloth {
	...
	constructor(offset: Vec3 = [0, 0, 0], particlesPerSide = 10, density = 1) {
		[this.particles, this.connections] = Cloth._allocateParticlesAndConnections(
			offset,
			particlesPerSide,
			density,
		);
		this.mesh = new Mesh(this.particles.length * 3, this.connections.length * 2, 'line');
		this._setIndices();
...
class InternalCollider {
	...
	pushOut(x: number, y: number, z: number): Vec3 {
		...
		return [
			this.pos[0] + dx * multiplier,
			this.pos[1] + dy * multiplier,
			this.pos[2] + dz * multiplier,
		];
	}
}
...
export class ClothSimSystem implements System {
	...
	protected colliders: Array<InternalCollider> = [];

	setup(world: World<Entity>): void {
		world
			.with('pos', 'collider')
			.onEntityAdded.subscribe(({ pos, collider }) =>
				this.colliders.push(new InternalCollider(pos, collider.radius)),
			);
	}

	run(world: World<Entity>, dt: number) {
		if (dt > this.maxDt) {
			dt = this.maxDt;
		}

		for (const { cloth } of world.with('cloth')) {
	...
	protected _solveConnectionConstraints(cloth: Cloth) {
		for (const c of cloth.connections) {
			...
			const dFactor = ((c.length - dlen) / dlen) * 0.5;
			const ox = dx * dFactor;
			const oy = dy * dFactor;
			const oz = dz * dFactor;
